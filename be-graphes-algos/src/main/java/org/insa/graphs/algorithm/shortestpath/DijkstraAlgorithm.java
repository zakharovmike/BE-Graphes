...
public class DijkstraAlgorithm extends ShortestPathAlgorithm {
	...
	protected ShortestPathSolution doRun() {
		...
		// Impossible node case
		if (destinationIndex > labels.length - 1 || originIndex > labels.length - 1) {
		...
		if (destinationIndex == originIndex) {
			...
			labels[destinationIndex].setPrevious(Node.linkNodes(this.graph.get(originIndex), this.graph.get(destinationIndex), 0, new RoadInformation(RoadType.MOTORWAY, null, true, 36, ""), null));
			path.add(labels[destinationIndex].getPrevious());
